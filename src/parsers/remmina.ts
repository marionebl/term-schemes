...
export function remmina(raw: any): TermScheme {
  if (!is.string(raw) || is.emptyOrWhitespace(raw)) {
    throw new TypeError(`remmina: input must be non-empty string`);
  }
...
function normalize(raw: any): [Error, null] | [null, RemminaScheme] {
  if (is.empty(raw)) {
    throw new TypeError(`remmina: input must be non-empty colors`);
  }

  if (!('ssh_colors' in raw)) {
    throw new TypeError(`remmina: expected ssh_colors group in colorscheme`);
  }

  const data = raw['ssh_colors'];
  ...
  REMINNA_KEYS
    .filter((key) => {
      if (!(key in data)) {
        errors.push(new TypeError(`remmina: missing "${key}"`));
        return false;
      }
      return true;
    })
    .filter((key) => {
      ...
      if (!is.string(val)) {
        errors.push(new TypeError(`remmina: expected "${key}" to be string, received "${typeof val}"`));
        return false;
      }
    ...
    .filter((key) => {
      ...
      if (!val.match(EXACT_HEX_MATCH)) {
        errors.push(new TypeError(`remmina: expected "${key}" to be hex color, received "${val}"`));
        return false;
      }
